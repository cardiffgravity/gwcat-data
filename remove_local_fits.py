#!/usr/bin/env python3
"""
Script to remove skymap-fits-local entries from the links section.
Removes entries where: type is "skymap-fits-local"
"""
...
def remove_skymap_fits_local(json_content):
    """
    Remove skymap-fits-local entries from the links section.
    ...
    for event_name, event_links in links.items():
        ...
        for link in event_links:
            ...
            if (isinstance(link, dict) and 
                link.get("type") == "skymap-fits-local"):
                count_removed += 1
                print(f"  Removing skymap-fits-local from {event_name}")
            else:
...
def main():
    ...
    # Remove the skymap-fits-local entries
    modified_content, count_removed = remove_skymap_fits_local(json_content)
    
    print(f"\nRemoved {count_removed} skymap-fits-local entries")
    
    if count_removed > 0:
        # Create output filename
        output_file = input_path.parent / f"{input_path.stem}_no_local_fits{input_path.suffix}"
