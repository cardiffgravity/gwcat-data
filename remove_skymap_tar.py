#!/usr/bin/env python3
"""
Script to remove skymap-fits entries with filetype='tar' from the links section.
Removes entries matching: {"text": "Sky Map", "type": "skymap-fits", "filetype": "tar"}
"""
...
def remove_skymap_tar_entries(json_content):
    """
    Remove skymap tar entries from the links section.
    ...
    for event_name, event_links in links.items():
        ...
        for link in event_links:
            ...
            if (isinstance(link, dict) and 
                link.get("text") == "Sky Map" and
                link.get("type") == "skymap-fits" and
                link.get("filetype") == "tar"):
                count_removed += 1
                print(f"  Removing skymap tar from {event_name}",link)
            else:
...
def main():
    ...
    # Remove the skymap tar entries
    modified_content, count_removed = remove_skymap_tar_entries(json_content)
    
    print(f"\nRemoved {count_removed} skymap tar entries")
    
    if count_removed > 0:
        # Create output filename
        output_file = input_path.parent / f"{input_path.stem}_no_skymap_tar{input_path.suffix}"
